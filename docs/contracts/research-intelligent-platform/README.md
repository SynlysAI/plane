...
- 新建课题挂到所属 `TEAM` 小组的共享知识库。小组绑定未回填为 `READY` 前，上传返回 `KB_NOT_READY`；课题没有小组时返回 `KB_GROUP_UNASSIGNED`，不创建个人库。
- 一个外部知识库只能绑定一个小组。同一小组的多个课题复用该库；绑定到另一个小组，或上传时使用其他小组的库，返回 `KB_SCOPE_CONFLICT`。已经 `READY` 的历史课题绑定不自动并库。
