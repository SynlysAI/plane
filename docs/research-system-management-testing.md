...
3. 打开 `/public/research/settings/system` → 「批量导入」上传两个文件 → 先点「预检」：
   - 预检结果列出逐行状态，**不写库**；
4. 审批导入后，下载「本批次导入报告」和导入区始终可见的「下载全部初始密码」：
   - 两行均可在导师映射和 TEAM 小组存在时导入；缺少映射或小组不唯一的行会标记待处理；
   - 新账号同时出现在批次报告和全员下载中，并要求首次登录修改密码；
   - 不勾选「为已有账号重新生成初始密码」时，再次导入不改变已有密码哈希，全员下载仍是上次初始密码，以及该账号当前真实的「仍须修改密码」状态；
   - 勾选后，已有账号才轮换密码，两处下载都变为新的初始密码，并重新要求修改密码。
5. 校验：学生挂到已有「石墨负极小组」TEAM 节点；首次登录时弹出「请设置你自己的密码」强制改密弹窗。全员下载不是当前密码，用户自己改密后不能从这里反查。
...
# 仅在明确要轮换已有账号时追加 --reset-passwords
```
...
| 首次登录弹窗要求改密                       | 这是最近一次发放的初始密码触发的强制改密。批次报告和「下载全部初始密码」都能看到它；改完标记消失。之后只能重新生成，不能查看当前密码 |
