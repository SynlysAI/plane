...
| 计划状态 | 规划中；需等待 Phase 1.5 灰度门禁完成后再启动                                                                                                                                                                                                                                                                                                                                             |
...
- 以 Phase 1 页面骨架为前置，不新增科研一级入口，不重排四入口信息架构。
- 在 Chain 节点显示 Job 状态、RunEvent、工具调用、审批和 DataAsset。
- 扩展 Phase 1 审批中心 `agent_approval` 队列，支持等待审批、已批准、已拒绝、审批中和关联 Job 状态。
...
- 将 Phase 1 专业组件占位替换为真实入口卡、健康状态、最近运行和跨组件待办；无法连接时保留降级和人工路径。
...
- Phase 1 状态矩阵、响应式布局、键盘可达性和权限过滤回归通过。
...
- [ ] Phase 1 四入口、页面骨架、状态矩阵、响应式和可访问性无大规模回归。
