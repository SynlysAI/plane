...
| 计划状态 | 规划中；前置依赖 Phase 2 和治理门禁，当前不排期                                                                                                                                                                                                                                                                                                     |
...
信息架构基线：Phase 3 继续沿用 `research_ia_v2` 四入口结构。治理提示、配额、审计跳转和策略配置分别嵌入研究链、审批中心和科研管理，不新增科研一级入口；只有当安全、合规或开放门禁需要临时回退时，才允许工作区管理员关闭 `research_ia_v2`。

UI 边界：

- 只叠加 scope banner、数据等级、伦理/IP 状态、策略命中、配额与用量、高风险确认、审计导出和写作/评审 Agent。
- 只对治理组件做小型视觉调整，不重构科研总览、研究链、审批中心或科研管理骨架。
...
- 审计与治理入口：策略配置和保留期进入科研管理；审批、阻断和高风险确认进入审批中心或节点上下文；Trace 和证据链留在研究链。
...
- 四入口、科研总览、研究链、审批中心、科研管理和 Agent 工作台骨架不被治理 UI 重构。
