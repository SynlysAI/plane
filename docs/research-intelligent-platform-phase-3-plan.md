...
信息架构基线：Phase 3 继续沿用 `research_ia_v2` 四入口结构。治理提示、配额、审计跳转和策略配置分别嵌入 研究链、审批中心和科研管理，不新增科研一级入口；只有当安全、合规或开放门禁需要临时回退时，才允许工作区管理员关闭 `research_ia_v2`。
...
- 审计与治理入口：策略配置和保留期进入科研管理；审批、阻断和高风险确认进入审批中心或节点上下文；Trace 和证据链留在 研究链。
