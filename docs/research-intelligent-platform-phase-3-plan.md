...
| 计划状态 | 待评审                                                                                                                                                                                                                                                                                                     |
| 目标     | 在 Phase 1 UI/UX Ready 工作台和 Phase 2 运行容器上，建立生产级科研治理、可复现、规模化、灾备和社会用户开放前置能力                                                                                                                                                                                         |
...
Phase 3 前置条件：Phase 1 已完成四入口与主要 Plane UX 容器，Phase 2 已在既有容器中填充真实运行能力。Phase 3 不承担大规模界面搭建，也不新增科研一级入口。

信息架构基线：Phase 3 继续沿用 `research_ia_v2` 四入口结构。治理提示、配额、审计跳转和策略配置分别嵌入 Research Chain、审批中心和科研管理，不新增科研一级入口；只有当安全、合规或开放门禁需要临时回退时，才允许工作区管理员关闭 `research_ia_v2`。

UI 边界：

- 只叠加 scope banner、数据等级、伦理/IP 状态、策略命中、配额与用量、高风险确认、审计导出和写作/评审 Agent。
- 只对治理组件做小型视觉调整，不重构科研总览、Research Chain、审批中心或科研管理骨架。
- 兼容路由删除必须独立评估和发布，不因 Phase 1 UX 前移而提前删除。
...
- 审计与治理入口：策略配置和保留期进入科研管理；审批、阻断和高风险确认进入审批中心或节点上下文；Trace 和证据链留在 Research Chain。
...
Phase 3 基于访问数据、外部链接依赖和迁移完成度评估是否删除 Phase 1/2 保留的旧列表兼容路由。删除前必须确认：

- 内部文档、书签、邮件和外部系统不再引用旧列表路由。
- 对象级深链仍保留权威地址或具备服务端跳转方案。
- 审计和访问日志能区分真实旧路由访问与自动化探测。
- 删除后有回滚方案，不影响 `research_ia_v2` 关闭时的应急回退路径。
...
### 4.6 Phase 1/2 UX 骨架回归

- 四入口、科研总览、Research Chain、审批中心、科研管理和 Agent 工作台骨架不被治理 UI 重构。
- 治理信息只叠加在对应节点、审批、管理或 Agent 区域，不新增科研一级入口。
- 高风险确认、策略命中、配额、审计跳转和写作/评审 Agent 均可回放决策链路。
- Phase 1 状态矩阵、响应式布局、键盘可达性和权限过滤在治理模式下回归通过。

## 5. 发布、回滚与风险

- 治理策略先以观察模式上线，再逐步切换为阻断模式。
- 兼容路由删除必须独立发布；异常时回滚路由层，不回滚治理数据和审计记录。
...
| 8    | 兼容路由治理            | Phase 1/2 使用数据   | 外部依赖可解释、删除/回滚方案通过    |
...
- [ ] 治理配置、审计导出和策略生命周期全部挂在科研管理容器内，不新增科研一级入口。
- [ ] 旧路由删除评估具备访问数据、外部依赖清单和独立回滚方案。
- [ ] Phase 1/2 四入口、页面骨架、状态矩阵、响应式和可访问性无大规模回归。
