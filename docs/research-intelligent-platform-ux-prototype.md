...
Research Chain 是“科研”模块的核心过程入口。`research_ia_v2` 默认开启，旧科研业务入口不再平铺展示，而是收敛为四个一级入口；权威对象、既有 API、导航能力 key 和对象 ACL 不因入口收敛而改变。开关关闭时，现有导航和页面行为完整保留。

### 2.0 当前界面问题与收敛结论

Phase 1 已完成 Chain、Agent、RAGPortal 和过程数据闭环，但界面仍按旧 Phase 1 计划平铺科研入口。当前截图暴露的问题收敛为四类：

- 侧栏同时暴露科研总览、报告、提交汇总、科研项目、科研链、待我评审、办公审批、组织架构和系统管理，用户需要理解管理域与过程域边界。
- 科研总览混合管理统计、项目表格、报告汇总、阶段评审和审批入口，页面职责过宽。
- Chain 首页、项目列表和报告列表缺少统一容器，用户无法感知“科研项目和报告属于 Research Chain 的保存视图”。
...
```text
科研总览
├─ 范围筛选 / 统计周期 / 刷新时间 / 进入 Research Chain
├─ 我的待办 · 待审批 · 待确认 · 阻塞与降级
├─ 我的课题摘要 · 报告提交汇总 · 近期成果
...
| Research Chain 列表   | 必做：课题、项目管理、报告与成果保存视图                                                       | 项目/报告视图补充专业运行和外部资产上下文         | 不重构骨架                         |
| Research Chain 课题页 | 必做：固定上下文条、七个页面 Tab、主链可视化、循环关系、成员、报告/实验/外部引用和回放         | 外部运行节点、DataAsset、自动 ELN 外部段          | 治理 gate 和结题/转化状态          |
...
- Phase 1：交付绝大部分 Plane 所属 UI/UX：四入口 IA、Plane 首页紧凑摘要、科研总览、Research Chain 课题页、六段式节点详情、Agent 自动装配工作台、六类快照、审批中心、科研管理、跨组件待办索引和基础状态矩阵。专业运行、治理和配额只保留真实占位/降级态。
